...
function Notification(){
    ...
    useEffect(() => {
        ...
        if(isAuthenticated && user){
            ...
            const fetchNotifications = async() => {
                try{
                    const response = await fetch(`${backendAddress}/notifications/${user.id}`, {
                        method: 'GET',
                        credentials: 'include',
                    });
    ...
    const deleteNotification = async(notificationId) => {
        try{
            const response = await fetch(`${backendAddress}/notifications/${notificationId}`, {
                method: 'DELETE',
                headers: {'Content-Type' : 'application/json'},
                credentials: 'include',
            });
    ...
    return(
        <div>
            ...
            {notifications.map((notification) => (
                <li key={notification.id} style={{backgroundColor: notification.read ? 'lightgrey' : 'white'}}>
                    ...
                    {notification.recipes && notification.recipes.map((recipe, index) => (
                        <div key={index}>
